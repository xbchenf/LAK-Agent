...
| `agent-complaint` | 投诉建议Agent | COMPLAINT_SUGGEST | 触发 SlotFillingEngine 多轮对话采集 + 工单创建 | `ComplaintAgent` |
...
**触发 Slot-Filling 多轮对话采集**，不直接返回静态引导。
...
public class ComplaintAgent implements SubAgent {
    ...
    public AgentResponse process(AgentRequest request) {
        // 触发 SlotFillingEngine.startFilling()，返回第一个槽位的追问话术
        String firstPrompt = slotFillingEngine.startFilling(request.getSessionId());
        return AgentResponse.builder()
                .answer(firstPrompt)
                .confidence(1.0)
                .intentType(IntentType.COMPLAINT_SUGGEST.name())
                .extra(Map.of("state", "COLLECT_INFO", "slotStage", "started"))
                .build();
    }
}
```

**设计决策变化**：v2.0 之前在 Agent 层仅返回静态引导页链接，实际采集由前端 TicketView 完成。v2.0 之后 ComplaintAgent 直接触发 `SlotFillingEngine.startFilling()`，后续轮次由 `ChatService` 检测 `COLLECT_INFO` 状态短路路由，不再经过意图分类和 Agent 调度。工单创建完成后引导至表单页面作为兜底（超轮次、LLM 不可用、用户主动要求时）。
...
| **COLLECT_INFO** | 投诉意图 | 短路路由：跳过意图分类，直接由 ChatService → SlotFillingEngine 处理。支持打断挂起（new_intent 路由到其他 Agent 回答，附回钩）、槽位修改、主动取消、超轮次转手工填写 |
...
## 7. Slot-Filling 引擎（FSM + LLM 混合架构）

**架构定位**：FSM 管流程（槽位推进、轮次限制、状态转换），LLM 管理解（噪声输入清洗、意图切换识别、槽值提取）。业界主流方案（GlobalDev 2025、IBM 2024、COLING 2025）。

### 7.1 槽位定义

| 序号 | 槽位名 | 标签 | 必填 | 追问话术 | 校验规则 | 规范化 |
|------|--------|------|------|---------|---------|--------|
| 0 | `complaintType` | 投诉类型 | ✅ | "请问您要反馈的是哪类问题？\n1.治安投诉\n2.窗口服务投诉\n3.派出所/民警投诉\n4.其他" | — | `normalizeComplaintType()`: "第3类"/"3"/"第三" → "派出所/民警投诉"，支持16种变体 |
| 1 | `contactName` | 联系人 | ✅ | "请留下您的称呼：" | — | 去口头前缀: "我叫"/"我是"/"我" |
| 2 | `contactPhone` | 联系电话 | ✅ | "请留下您的联系电话：" | `^1[3-9]\d{9}$` | 正则直提优先，无需LLM |
| 3 | `description` | 问题描述 | ✅ | "请详细描述您的问题：" | — | — |
| 4 | `attachment` | 附件 | ❌ | "如有相关材料可上传（可选），没有请回复 无 ：" | — | — |

### 7.2 状态存储

Redis Hash `session:{sessionId}` 字段：

| Key | 类型 | 说明 |
|-----|------|------|
| `slotValues` | JSON Map | 已收集的槽位键值对 |
| `currentSlot` | int | 当前槽位索引 (0-4) |
| `fillRound` | int | 已进行轮数，超过 MAX_FILL_ROUNDS(5) 触发 redirectToManual |
| `nlSummary` | String | 自然语言摘要（NL-DST），作为 LLM 上下文："已填：投诉类型：治安投诉，联系人：张三。待填：联系电话、问题描述。" |
| `interruption` | JSON | `{active, rounds, resumeSlot}` — 打断状态，rounds ≥ 3 强制回归 |

### 7.3 采集流程（FSM + LLM 混合管道）

```
processResponse(sessionId, userMessage):  // ChatService.handleCollectInfo() 调用
  │
  ├─ Step 0: 恢复打断状态（如用户回到工单话题，清除 interruption）
  │
  ├─ Step 1: 规则预清洗
  │   ├─ 去口头前缀: /^[我我们]?是?叫?/
  │   ├─ 手机号正则直提: /(1[3-9]\d{9})/
  │   ├─ 取消检测: "算了"/"不填了"/"取消" → action=cancel
  │   └─ 提交检测: 必填齐全 + "提交"/"好了" → completeFilling
  │
  ├─ Step 2: LLM 结构化抽取 (extractWithRetry)
  │   ├─ Prompt: 槽位标签 + NL摘要 + 用户消息 → 要求输出JSON
  │   ├─ 输出: {action, targetSlot, extractedValue, confidence, reasoning}
  │   └─ JSON校验失败 → reprompt × 2 → 仍失败 → 降级 fallbackExtraction()
  │
  ├─ Step 3: 投诉类型规范化 (LLM 和规则路径都做)
  │
  └─ Step 4: 按 action 分发
      ├─ fill       → 写槽值 → 推进下一槽位 → 更新 NL 摘要
      ├─ modify     → 回退到指定槽位重新询问
      ├─ chitchat   → 挂起 (interruption:{active:true}) → 回答并附回钩
      ├─ new_intent → 挂起 → ChatService 路由到 PolicyAgent/ProcedureAgent
      ├─ cancel     → transition CLOSED
      └─ (兜底)      → 超5轮 / LLM不可用 → redirectToManual
```

### 7.4 LLM 抽取与降级

```java
// extractWithRetry: LLM调用 → JSON校验 → 失败重试×2 → 降级
SlotExtractionResult extractWithRetry(sessionId, msg, cleaned, phoneDirect) {
    for (int attempt = 0; attempt < 3; attempt++) {
        try {
            String raw = chatModel.chat(prompt);              // LLM调用
            SlotExtractionResult r = validator.parseAndValidate(raw, slotName); // JSON校验
            return r;
        } catch (ExtractionParseException e) { /* reprompt, 最多2次 */ }
          catch (Exception e) { /* LLM不可用 → 立即降级 */ return fallbackExtraction(...); }
    }
    return fallbackExtraction(msg, cleaned, slotName, phoneDirect);
}

// fallbackExtraction: LLM 不可用时的规则降级
├─ 取消/提交关键词 → cancel/fill
├─ 闲聊关键词(天气/你好/谢谢) → chitchat
├─ 新意图关键词(违法/怎么办/法律规定) → new_intent
├─ 修改关键词(改/错了/不对) → modify
├─ 手机号直提(contactPhone槽位+正则命中) → fill
├─ 投诉类型规范化(complaintType槽位) → fill
└─ 默认 → fill(用规则清洗后的文本)
```

### 7.5 FillingResult（扩展后）

```java
public record FillingResult(
    boolean done,
    String nextPrompt,
    String retrySlot, String retryMessage,
    Map<String, String> slotValues,
    String resultType,       // fill|modify|chitchat|new_intent|cancel|interrupted|redirect|done
    String interruptReason,  // 中断原因 — 供 ChatService 路由判断
    SlotDefinition interruptedSlot
) {
    static FillingResult nextSlot(String prompt);
    static FillingResult retry(String slot, String msg);
    static FillingResult allDone(Map<String, String> values);
    static FillingResult timeout(int maxRounds);
    static FillingResult cancelled();
    static FillingResult interrupted(String reason, SlotDefinition slot);
    static FillingResult redirectToManual(Map<String, String> prefilledSlots, String message);
}
```
...
| 决策 | 理由 |
|------|------|
| FSM 管流转，LLM 管提取 | 流程可控（轮次上限、状态转换），避免纯LLM自主决策的漂移 |
| NL 摘要替代键值对作 LLM 上下文 | NL-DST 论文（2025）表明 LLM 对自然语言摘要理解准确率显著高于键值对 |
| JSON Schema 强制校验 | 防止 LLM 输出格式错误导致槽位污染（GlobalDev 2025 最佳实践） |
| LLM 不可用时规则降级 | 不因大模型故障瘫痪对话创建工单的核心能力 |
| 超5轮 redirectToManual | 不强行完成不完整的工单，引导用户到表单页面补全 |
| 投诉类型枚举映射 | "第3类"/"3" → "派出所/民警投诉" — 规范化确保工单数据质量 |
...
ChatService.processMessageStream(userId, sessionId, message)
  │
  ├─ Step 1: COLLECT_INFO 短路检测
  │   └─ status == COLLECT_INFO → handleCollectInfoStream() → 槽位处理 → done
  │
  ├─ Step 2-3: 同 JSON 模式 — 主Agent路由决策
  │
  ├─ Step 4: 路由分发
  ...
  │   └─ COMPLAINT → 触发 SlotFilling → SSE message + done(extra={state,slotStage})
  │
...
- **COLLECT_INFO 状态**：槽位采集消息不涉及知识检索，跳过 COMPLIANCE_CHECK
- **ComplaintAgent 答复**：投诉引导初始消息不涉及知识检索
...
ChatService.processMessage(userId, sessionId, message)
  ...
  │
  ├─ [状态检查] sessionManager.getStatus(sessionId)
  │   ├─ COLLECT_INFO → 短路路由 → handleCollectInfo() [见 §7]
  │   └─ 其他状态 → 继续正常流程
  │
  ...
  │               └─ 触发 SlotFillingEngine.startFilling() → 进入 COLLECT_INFO
  │
  ├─ [合规校验] (COLLECT_INFO 时跳过)
  │   session → COMPLIANCE_CHECK
