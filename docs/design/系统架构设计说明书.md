...
**职责**：面向政法干警的 Web 交互界面，提供智能问答对话、工单提交、历史记录查询、管理后台等功能。作为用户与 AI Agent 平台之间的交互桥梁，必须满足政务内网部署、私有化静态资源、无障碍访问等要求。

#### 2.2.1 技术栈

| 组件 | 选择 | 版本 | 选型理由 |
|------|------|------|---------|
| **核心框架** | Vue 3 | 3.5+ | Composition API + TypeScript 原生支持，生态成熟，国内政企项目主流选择 |
| **构建工具** | Vite | 6.x | 开发体验极快，Vue 官方推荐，替代 webpack |
| **UI 组件库** | Element Plus | 2.9+ | Vue 3 最成熟的企业级 UI 库，中文文档完善，政务/政企项目广泛使用 |
| **状态管理** | Pinia | 2.x | Vue 3 官方推荐状态管理，TypeScript 友好，替代 Vuex |
| **路由** | Vue Router | 4.x | Vue 3 官方路由 |
| **HTTP 客户端** | Axios | 1.x | 拦截器机制完善（Token 注入、统一错误处理），政企项目标配 |
| **SSE 客户端** | @microsoft/fetch-event-source | 2.x | 流式对话需要 SSE，比原生 EventSource 支持 POST + 自定义 Header |
| **Markdown 渲染** | markdown-it + highlight.js | — | AI 答复含 Markdown 格式（溯源引用、列表），需安全渲染（防 XSS） |
| **图标库** | @element-plus/icons-vue | — | 与 Element Plus 配套，无需额外引入第三方图标库 |
| **工具函数** | @vueuse/core | 12.x | Vue 3 Composition API 工具集（键盘事件、剪贴板、本地存储等） |
| **TypeScript** | TypeScript | 5.x | 类型安全，与后端 Java 强类型体系匹配 |
...
| 方案 | 为何不选 |
|------|---------|
| React + Ant Design | Vue 在国内政企项目占比远超 React，团队招聘和后续维护成本更低 |
| Nuxt 3（SSR） | 政务内网私有化部署不需要 SEO，纯 SPA 即可；SSR 增加服务端 Node.js 依赖 |
| TDesign / Naive UI | Element Plus 政企项目案例最多、社区最活跃、政务项目甲方接受度最高 |
| 自研组件库 | 开发周期不可控，且无法享受社区的安全修复和浏览器兼容性适配 |
...
├── src/
│   ├── api/                    # 后端 API 调用封装
│   │   ├── auth.ts             #   登录/验证码/刷新Token
│   │   ├── chat.ts             #   对话消息/会话列表/会话详情
│   │   ├── ticket.ts           #   工单创建/工单查询
│   │   └── admin.ts            #   敏感词管理/知识库管理
│   ├── assets/                 # 静态资源（图标、图片）
│   ├── components/             # 通用组件
│   │   ├── chat/               #   对话相关组件
│   │   │   ├── ChatWindow.vue      # 对话窗口（消息列表+输入框）
│   │   │   ├── MessageBubble.vue   # 消息气泡（区分 user/assistant）
│   │   │   ├── SourceCitation.vue  # 溯源引用卡片（文档来源+原文片段）
│   │   │   └── StreamingText.vue   # SSE 流式文本逐字渲染
│   │   ├── common/             #   通用组件
│   │   │   ├── AppHeader.vue       # 顶部导航
│   │   │   ├── AppSidebar.vue      # 侧边栏（会话列表）
│   │   │   └── SensitiveWordDialog.vue  # 敏感词拦截提示
│   │   └── ticket/             #   工单组件
│   │       └── TicketForm.vue      # 工单填写表单
│   ├── composables/            # Composition API 复用逻辑
│   │   ├── useChat.ts          #   对话逻辑（发送/接收/SSE流式）
│   │   ├── useAuth.ts          #   认证逻辑（登录/登出/Token刷新）
│   │   └── useSensitiveWord.ts #   敏感词检测提示
│   ├── layouts/                # 布局组件
│   │   ├── DefaultLayout.vue   #   默认布局（Header + Sidebar + Main）
│   │   └── AdminLayout.vue     #   管理后台布局
│   ├── router/                 # 路由配置
│   │   └── index.ts            #   路由守卫（登录校验）+ 页面路由
│   ├── stores/                 # Pinia 状态管理
│   │   ├── auth.ts             #   用户认证状态
│   │   ├── chat.ts             #   对话状态（当前会话、消息列表）
│   │   └── app.ts              #   全局应用状态（侧边栏折叠等）
│   ├── types/                  # TypeScript 类型定义
│   │   ├── api.ts              #   API 请求/响应类型（与后端 DTO 对齐）
│   │   ├── chat.ts             #   消息/会话/溯源类型
│   │   └── ticket.ts           #   工单类型
│   ├── utils/                  # 工具函数
│   │   ├── request.ts          #   Axios 实例 + 拦截器配置
│   │   ├── sse.ts              #   SSE 连接管理（重连/超时）
│   │   └── markdown.ts         #   Markdown 安全渲染（XSS 过滤）
│   ├── views/                  # 页面组件
│   │   ├── LoginView.vue       #   登录页
│   │   ├── ChatView.vue        #   对话主页
│   │   ├── TicketView.vue      #   工单查询页
│   │   └── admin/              #   管理后台页面
│   │       ├── DashboardView.vue       # 仪表盘
│   │       ├── SensitiveWordsView.vue  # 敏感词管理
│   │       └── KnowledgeView.vue       # 知识库管理（P1）
│   ├── App.vue
│   └── main.ts
...
用户输入消息 → 敏感词前置校验(前端)
  → 命中敏感词 → 弹出提示，消息不发送
  → 通过
    → POST /api/v1/chat/message (Accept: text/event-stream)
      → SSE 流式接收 AI 答复（逐 Token 渲染）
        → 答复完成，渲染溯源引用卡片（来源文档 + 原文片段）
          → 用户点击溯源卡片 → 展开原文引用详情
```
...
| 场景 | 策略 |
|------|------|
| AI 正在生成 | `StreamingText.vue` 逐字追加渲染，光标闪烁效果 |
| 生成完成 | `MessageBubble.vue` 整体渲染 Markdown（标题/列表/链接/溯源标注） |
| 网络中断 | SSE `onerror` 触发，显示"连接中断，点击重试"按钮 |
| 敏感词后置命中 | 后端返回 error 事件，前端清空已渲染的流式内容，显示"系统繁忙" |

**管理后台模块**：

| 页面 | 功能 | API |
|------|------|-----|
| 仪表盘 | 今日对话量、意图分布、置信度分布、大模型调用 Token 消耗 | （P1 补充统计接口） |
| 敏感词管理 | 敏感词列表、新增/删除、热加载触发 | POST /api/v1/admin/sensitive-words/reload |
| 知识库管理 | 文档上传、分块状态查看、检索效果评估 | （P1 补充） |

#### 2.2.5 前端安全要求

| 要求 | 实现方式 |
|------|---------|
| **XSS 防护** | markdown-it 配置 `html: false`，禁止原始 HTML 渲染；DOMPurify 二次清洗 |
| **Token 安全** | Access Token 仅存内存（Pinia），不存 localStorage/sessionStorage；Refresh Token 存 httpOnly Cookie |
| **敏感词前端拦截** | 与后端共享同一份敏感词库文件（部署时挂载），输入时实时检测 |
| **私有化部署** | `vite build` 产出纯静态文件（HTML/JS/CSS），无外部 CDN 依赖，所有依赖打包至 `dist/` |
| **CSP 策略** | Nginx 配置 Content-Security-Policy 头，禁止 inline script |

#### 2.2.6 与后端的分工边界

```
┌── 前端职责 ──────────────────────────────────────┐
│ 页面渲染 / 路由跳转 / 表单校验 / SSE 流式渲染      │
│ 敏感词前端预拦截 / Token 管理 / Markdown 渲染     │
└──────────────────────┬──────────────────────────┘
                       │ HTTPS + JWT Bearer Token
┌──────────────────────┴──────────────────────────┐
│ 后端职责                                          │
│ 认证鉴权 / 敏感词后置拦截 / 意图识别 / RAG检索     │
│ 大模型调用 / 合规校验 / 审计日志 / 工单创建         │
└─────────────────────────────────────────────────┘
```

> 前端不做：意图识别、模型调用、合规判断、审计日志写入。前端仅做 UI 层的安全预拦截（敏感词），真正的安全防线在后端。
...
#### 2.4.1 主Agent（MasterAgent）

**核心组件**：

| 组件 | 职责 | 输入 | 输出 |
|------|------|------|------|
| IntentClassifier | 调用大模型进行意图分类（结构化 JSON 输出） | 用户消息 + 对话上下文（最近3轮） | 意图类别 + 自评置信度 + 分类依据 |
| ConfidenceEvaluator | 多层置信度融合（结构化输出 + 规则加权 + 投票/降级） | 分类结果 + 用户消息 | 最终置信度分值 (0.0-1.0) |
| RouteDispatcher | 根据意图+置信度决策路由目标 | 意图类别 + 最终置信度 | 路由目标 (AgentA/B/C 或 兜底处理器) |
...
> 背景：百炼 Qwen3.7-Max API **不支持**返回 token 级 LogProbs。因此不能依赖模型内部概率，采用业界主流的 **结构化输出 + 规则加权 + Self-Consistency 降级** 三层组合方案。

**第一层：结构化输出（主方案，成本 1×）**

通过 Prompt 要求模型以 JSON 格式同时输出意图和自评置信度，利用 Few-Shot 示例校准置信度分布：
...
Few-Shot 示例：
用户: "《行政复议法》第9条规定的申请期限是多长"
输出: {"intent":"POLICY_CONSULT","confidence":0.95,"reasoning":"明确引用了具体法规名称和条款号"}

用户: "怎么办护照"
输出: {"intent":"CHITCHAT","confidence":0.85,"reasoning":"护照办理不属于政法业务范围"}

用户: "我想反映个问题，小区物业乱收费"
输出: {"intent":"COMPLAINT_SUGGEST","confidence":0.75,"reasoning":"涉及投诉反映问题"}

用户: "那个事怎么弄"
输出: {"intent":"UNKNOWN","confidence":0.15,"reasoning":"信息严重不足，无法判断意图"}
```

**第二层：规则层加权（Rule Boost / Penalty）**

在模型输出的原始置信度基础上，应用政法领域先验规则：

| 规则 | 触发条件 | 调整值 | 说明 |
|------|---------|--------|------|
| 政法术语 Boost | 命中预定义高频术语表（"行政复议"、"执法程序"、"工伤认定"等 200+ 词） | +0.05 | 只作用于匹配到的意图类别 |
| 发文字号精确命中 | 正则命中 "X政发〔20XX〕XX号" 模式 | 置信度 ≥ 0.85，意图锁定 POLICY_CONSULT | 硬规则，直接覆盖 |
| 投诉关键词命中 | 命中 "我要投诉"／"我要举报"／"不作为" 等 | 置信度 ≥ 0.85，意图锁定 COMPLAINT_SUGGEST | 硬规则，直接覆盖 |
| 消息过短惩罚 | 用户消息 < 5 字 且 无政法术语命中 | −0.10 | 信息太少，置信度降低 |
| 结构标记 Boost | 命中 "第X条"／"第X章" 等法规结构标记 | +0.08 | 强信号，意图大概率是政策咨询 |

规则层与模型层的组合：

```java
// 硬规则：精确匹配直接返回高置信度
if (exactMatchRegex.matches(userMessage)) {
    return ConfidenceResult.of(intent, clamp(0.85 + ruleBoost, 0.0, 1.0));
}
// 软规则：模型分数 + 规则加权
...
| 触发条件 | 策略 | 额外成本 | 说明 |
|---------|------|---------|------|
| 模型调用超时 / 失败 | **Embedding 降级**：计算查询向量与各意图质心向量的余弦相似度 → Softmax 归一化为置信度 | +1 次 Embedding API | 不依赖大模型，纯向量方案 |
| 置信度 0.5~0.7（灰色地带） | **Self-Consistency 投票**：同一问题调 3 次（temperature=0.3），取多数一致的比例 | +2 次大模型调用 | 一致性提供额外证据 |
| 模型置信度 > 0.9 但 Embedding 相似度 < 0.5 | **矛盾告警**：记录审计日志标记 `confidence_mismatch`，降级至灰色地带处理 | 0 | 防止模型"幻觉自信" |

**置信度路由决策**：

```
模型结构化输出 → 规则加权 → 修正后的基线置信度
  │
  ├── ≥ 0.7 ──→ 直接路由至对应子Agent
  │
  ├── 0.5~0.7 ──→ Self-Consistency 投票（3次）
  │                ├── 投票 ≥ 0.7 → 路由
  │                └── 投票 < 0.7 → 兜底
  │
  └── < 0.5 ──→ 直接兜底，返回人工客服引导
...
| 置信度区间 | 动作 | 大模型调用次数 |
|-----------|------|--------------|
| ≥ 0.7 | 直接路由到对应子Agent | 1 次（意图分类） |
| 0.5 ~ 0.7 | 触发 Self-Consistency 投票 | 1 + 3 = 4 次 |
| < 0.5 | 直接兜底，返回人工客服引导 | 1 次（仅分类） |
| 大模型不可用 | Embedding 质心降级判定 | 1 次 Embedding |
...
| 准备项 | 说明 |
|--------|------|
| Few-Shot 校准 | 准备 50~100 条标注数据，反复调 Prompt 直到模型输出的置信度分布合理（不能全部 0.9+） |
| 意图质心向量 | 为每个意图类别准备 20+ 条典型表述，预计算 Dense Vector 质心 |
| 政法术语词表 | 200+ 条政法高频术语，每季度更新 |
| 阈值调优 | 0.7 / 0.5 两个阈值上线后通过 A/B 测试或人工标注数据校准 |
| 百炼 API Key 冗余 | 至少配置 2 个 Key，避免限流导致 Self-Consistency 投票失败 |

**决策理由**：对低置信度场景采用保守策略——宁可拒答引导人工，也不可给出可能错误的AI答复。这是政法场景的合规底线。三层方案在不依赖 LogProbs 的前提下，通过结构化输出获得基线置信度，用规则层弥补模型对政法术语的认知偏差，在灰色地带用投票法二次确认——以最小的额外成本换取置信度的可靠性。

#### 2.4.2 子Agent调度器（SubAgentScheduler）
...
#### 2.5.1 RAG引擎（RagEngine）

**核心组件**：

| 组件 | 职责 | 技术实现 |
|------|------|---------|
| DocumentParser | 多格式文档解析（PDF/Word/HTML） | Apache Tika / PDFBox + POI |
| DocumentChunker | 文档智能分段（政法场景语义分段） | 基于文档结构的段落拆分 + 滑动窗口重叠 |
...
```
原始文档 (PDF/Word/HTML)
  → DocumentParser.parse()               // 解析为纯文本 + 提取元数据
    → DocumentChunker.chunk()             // 智能分段
      → EmbeddingService.embed(chunk)     // 生成 Dense Vector
      → SparseEmbeddingService.encode()   // 生成 Sparse Vector
        → Qdrant.upsert(point)            // 双向量入库
          → MySQL.insert(metadata)        // 元数据落库
```

### 文档分块策略（Document Chunking Strategy）

政法文档（政策法规、办事指南）具有天然的结构化特征——条、款、项、目层级分明，语义边界清晰。分块策略必须**利用这一结构特征**，而非简单按固定字符数切分。

#### 分块方案：结构感知段落分块（Structure-Aware Paragraph Chunking）

| 策略维度 | 选择 | 理由 |
|---------|------|------|
| **分块方式** | **语义段落分块**（按条/款/章节边界） | 政法文档的\"条\"本身就是完整的语义单元，不应被切断 |
| **块大小（主）** | 500-1000 字符 | text-embedding-v4 最优编码区间，且与\"一条\"的典型长度匹配 |
| **块大小（最小）** | 200 字符 | 短条款合并至上一块，避免碎片化 |
| **块大小（最大）** | 1500 字符 | 长条款二次切分，在段落边界处断开 |
| **重叠策略** | 前向重叠 100-200 字符（约1-2句话） | 防止检索时切断跨块语义，政法文档条款间常有引用关系 |
| **特殊处理** | 表格/流程图：保留为独立块 + 文本描述摘要 | 表格数据向量检索效果差，需摘要辅助 |

**分级分块逻辑**：

```
Level 1 — 文档级元数据提取
  提取: 文档标题、发文字号、发布机构、发布日期、生效日期、废止日期

Level 2 — 结构切分（按章节/部分/条）
  识别: 正则匹配 "第X条" "第X章" "第X节" 等政法文档结构标记
  策略: 以"条"为基本分块单元，章/节信息注入到每个块的元数据中

Level 3 — 语义完整性保护
  检查: 切分点是否在完整句子边界
  规则: 绝不在句子中间切分 → 宁可块略大也不断句

Level 4 — 重叠窗口
  策略: 每个块保留前一块的最后 1-2 个句子作为上下文前置
  目的: 在 RAG 检索时，即使命中单一 chunk，也能看到上下文过渡
```
...
| 参数 | 建议值 | 环境变量 |
|------|--------|---------|
| 主块大小 | 800 字符 | `LAK_CHUNK_SIZE=800` |
| 最小块大小 | 200 字符 | `LAK_CHUNK_MIN_SIZE=200` |
| 最大块大小 | 1500 字符 | `LAK_CHUNK_MAX_SIZE=1500` |
| 重叠字符数 | 150 字符 | `LAK_CHUNK_OVERLAP=150` |
| 结构标记正则 | `第[一二三四五六七八九十百]+[章条款项]` | `LAK_CHUNK_PATTERN` |

#### 为什么不用纯固定大小分块

| 场景 | 固定大小分块（如 LangChain RecursiveCharacterTextSplitter） | 本方案的语义段落分块 |
|------|----------|----------|
| "第十条"横跨两个块 | ❌ 条款被腰斩，检索结果语义不完整 | ✅ 以条为边界，完整保留 |
| 短条款（< 200字） | ⚠️ 可能独立成块，上下文丢失 | ✅ 自动合并至相邻块 |
| 表格/流程图 | ❌ 纯文本截断，表格无法检索 | ✅ 独立保留 + 文本摘要 |
| 发文字号跨块 | ❌ "X政发〔2024〕" 和 "12号" 分在两端 | ✅ 作为文档级元数据常驻每个块 |

> **两类文档的分块差异**：上述策略以政策法规的"条"为锚点示例。办事指南文档没有"第X条"结构标记，但同样有清晰的语义边界——章节标题（"一、"、"办理条件："等）。架构层面确认两类文档需要**不同的切割锚点**，具体正则、配置参数和表格处理策略属于 P1 `知识库与RAG详细设计` 的范围。
...
| SlotFillingEngine | 多轮信息补齐（投诉场景） | FSM管流程 + LLM管提取：预清洗→LLM抽取→JSON校验→动作分发。支持打断挂起、槽位修改、LLM降级规则兜底、超轮次转手工填写 |
...
| COLLECT_INFO | 意图 = COMPLAINT_SUGGEST | 短路路由（跳过意图分类），SlotFillingEngine FSM+LLM 混合管道处理。支持打断挂起（new_intent路由到其他Agent）、槽位修改、超轮次转手工填写 |
...
**Slot-Filling 流程（FSM + LLM 混合管道）**：

```
processResponse(sessionId, userMessage):
  ① 规则预清洗 → 去口头前缀、手机号直提、取消/提交关键词
  ② LLM结构化抽取 → JSON{action, targetSlot, extractedValue, confidence}
  ③ JSON Schema校验 → 失败reprompt×2 → 仍失败降级规则提取
  ④ 动作分发:
     fill    → 写槽值 → 推进槽位 → 更新NL摘要
     modify  → 回退指定槽位
     chitchat/new_intent → 挂起(interruption) → 路由到目标Agent
     cancel  → CLOSED
     超5轮   → redirectToManual (转手工填写)
```
...
| 槽位字段 | 类型 | 必填 | 采集方式 | 规范化 |
|---------|------|------|---------|--------|
| complaintType | 枚举(4选1) | 是 | LLM提取 + 规则映射 | normalizeComplaintType: "第3类"/"3" → "派出所/民警投诉" |
| contactName | 文本 | 是 | LLM提取，去"我叫/我是/我"前缀 | — |
| contactPhone | 手机号 | 是 | 正则直提优先，LLM兜底 | `^1[3-9]\d{9}$` |
| description | 长文本 | 是 | LLM识别填充 | — |
| attachment | 文件(可选) | 否 | 引导上传或回复"无" | — |

- 最大轮数：**5 轮**，超限 → redirectToManual（携带已填数据跳转手工表单页）
- 打断上限：**3 轮**，超限强制回归槽位
...
Fields:
  ...
  - fillRound       (当前补齐轮数)
  - nlSummary       (自然语言摘要 — NL-DST，作为LLM上下文)
  - interruption    (JSON: {active, rounds, resumeSlot} — 打断状态)
  - createTime
  - lastActiveTime
TTL: 1800 秒 (30 分钟)
```

#### 2.5.3 工单适配器（TicketAdapter）
...
#### 2.5.4 合规校验器（ComplianceValidator）
...
**Qdrant Point Payload Schema**（每个 chunk 存储一条 Point，Payload 承载分块元数据与溯源信息）：
...
```json
{
  ...
  "fragment_text": "第十五条 行政执法机关在作出行政处罚决定之前，应当告知当事人...",
  "overlap_prefix": "...前款规定适用于所有行政处罚种类。",
  "keywords": ["行政处罚", "告知义务", "陈述申辩"],
  "version": 1
}
```
...
| doc_type | keyword | POLICY / PROCEDURE / TEMPLATE | Collection 内按类型过滤 |
| source_no | keyword | 发文字号 | Sparse 精确匹配 |
| publish_dept | keyword | 发布机构 | 按部门过滤检索 |
| effective_date | datetime | 生效日期 | 时效性过滤（只检索有效政策） |
| expire_date | datetime | 废止日期 | 过期文档排除 |
| chapter | keyword | 所属章节 | 结构化溯源展示 |
| article_no | keyword | 条号 | 精确条文定位 |
| chunk_index | integer | 块序号 | 相邻块导航 |
| total_chunks | integer | 文档总块数 | 完整性校验 |
| prev_chunk_id | keyword | 上一块 ID | 上下文扩展检索 |
| next_chunk_id | keyword | 下一块 ID | 上下文扩展检索 |
| fragment_text | text | 块的完整文本 | 大模型上下文注入 + SourceTracer 溯源 |
| overlap_prefix | text | 前块重叠文本 | chunk 间语义桥接 |
| keywords | keyword[] | 关键词列表 | Sparse 向量增强 + Payload 过滤 |
| version | integer | 版本号 | 知识更新时标记新版本 |
