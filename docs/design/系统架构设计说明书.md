...
| 组件 | 职责 | 技术实现 |
|------|------|---------|
| EmbeddingService | 将文本转换为向量 | 调用百炼 text-embedding-v4 API |
| VectorRetriever | 向量相似度检索 | Qdrant 客户端 + LangChain4j EmbeddingStoreRetriever |
| KnowledgeRepo | 知识库元数据与内容查询 | MySQL（元数据）+ Qdrant（向量） |
| Reranker | 检索结果重排序 | 基于 BM25 + 语义相似度加权融合 |
...
用户问题
  ...
  → VectorRetriever.search(vector, topK=10) // Qdrant 粗排召回 Top-10
  → Reranker.rerank(query, top10Results)   // 重排序 → Top-5
...
| 参数 | 建议值 | 说明 |
|------|--------|------|
| Top-K 粗排 | 10 | Qdrant 向量检索返回量 |
| Top-K 精排 | 5 | 重排序后送入大模型上下文的片段数 |
| 相似度阈值 | ≥ 0.75 | 低于此分数的片段不送入大模型 |
| 最大检索超时 | 3s | 超过此时间触发降级 |
| Embedding 维度 | 1536 | text-embedding-v4 输出维度 |
...
**职责**：存储知识文档的向量 Embedding，提供高性能相似度检索。

**Collection 设计**：

| Collection 名称 | 使用方 | 向量维度 | 距离度量 |
|----------------|--------|---------|---------|
| `lak_policy_docs` | 子AgentA（政策咨询） | 1536 | Cosine |
| `lak_procedure_docs` | 子AgentB（办事指引） | 1536 | Cosine |
...
| **过滤能力** | 支持 Payload 过滤 | 支持标量过滤 | **最强**——全文搜索 + 聚合 |
| **政法场景适配** | 部署简单、资源可控、Rust 内存安全 | 功能最强但运维成本高 | 有全文检索优势但向量检索非长项 |
| **最终选择** | **Qdrant** —— 政法私有化部署场景下，部署简单、资源消耗低、运维友好是首要考量。当前阶段知识库规模在百万级，Qdrant 的单节点性能完全满足。 |

**备选切换路径**：如未来需要全文检索+向量检索混合场景，可引入 Elasticsearch 作为关键词检索补充，Qdrant 专注向量检索。
