...
│   ├── controller/                              # 控制器层
│   │   ├── AuthController                       #   /api/v1/auth/*（含注册）
│   │   ├── ChatController                       #   /api/v1/chat/*（含 WebSocket SSE 推送）
│   │   ├── TicketController                     #   /api/v1/tickets/*
│   │   ├── OperatorController                   #   /api/v1/operator/*（坐席工单+会话）
│   │   ├── KnowledgeController                  #   /api/v1/knowledge/*
│   │   └── AdminController                      #   /api/v1/admin/*（含敏感词列表）
│   ├── service/                                 # 业务逻辑层
│   │   ├── agent/                               #   Agent编排
...
│   │   ├── chat/                                #   坐席服务
│   │   │   ├── OperatorChatService              #     接管/消息/关闭人工会话
│   │   │   ├── ChatWebSocketHandler             #     WebSocket 双向通讯
│   │   │   ├── MessagePushService               #     消息推送（委托 WebSocket）
│   │   │   └── HandoffSummaryGenerator          #     LLM 转人工摘要
│   │   ├── agent/master/                        #   新增
│   │   │   ├── HandoffSummaryService            #     转人工摘要 @AiService
│   │   │   └── FallbackHandler                  #     三步递进兜底策略
│   │   ├── knowledge/                           #   知识库文档管理
│   │   ├── ticket/                              #   工单模块
│   │   │   ├── TicketAdapter                    #     工单CRUD + 全量查询
│   │   │   └── OperatorTicketService            #     坐席工单服务
│   │   ├── audit/                               #   审计日志
│   │   └── security/                            #   安全服务
│   ├── config/                                  # 配置类
│   └── resources/
...
`NEW` → `INTENT_CHECK` → `ANSWERING` | `COLLECT_INFO` | `FALLBACK` | `WAITING_OPERATOR` | `HUMAN_HANDLING` → `COMPLIANCE_CHECK` | `TICKET_SUBMIT` → `CLOSED`
...
| FALLBACK | 置信度<0.5 | 三步递进：第1次引导重述 → 连续2次确认转人工 |
| WAITING_OPERATOR | 用户确认/主动请求转人工 | Redis List 排队，用户端显示排队位置 |
| HUMAN_HANDLING | 坐席点击"接入" | WebSocket 双向通讯，坐席接管会话 |
| COMPLIANCE_CHECK | 答复生成完毕 | 合规校验（COLLECT_INFO时跳过） |
| TICKET_SUBMIT | 槽位全部填完 | TicketAdapter.createTicket() |
| CLOSED | 关闭/超时/取消 | Redis TTL 1800s，新消息自动创建新会话 |

### 转人工链路

```
用户消息
  ├─ 用户主动"转人工" → 直接入队列 WAITING_OPERATOR
  ├─ 连续2次低置信度 → 确认后入队列
  ├─ 低置信度 <0.5 → 第1次引导重述，不提转人工
  └─ 聊天框快捷按钮：👤 转人工 / 📮 投诉建议
```

**触发方式**：`IntentClassifier` 硬规则关键词匹配 / `ChatService` 兜底计数器 / `FallbackHandler` 三步递进策略。

**WebSocket 通讯**：坐席对话使用 `ChatWebSocketHandler`（`/ws/chat` 端点）。用户端和坐席端各自建立 WebSocket 连接，消息通过 Handler 双向转发。AuthFilter 白名单放行 WebSocket 握手路径。

**坐席工作台**：`/operator` 页面，展示 4 张统计大卡 + 等待接入/我的会话面板 + 工单列表表格（含状态筛选 Tab）。每 5 秒轮询刷新等待队列。

**槽位简化**：投诉流程从 5 槽减为 3 槽（投诉类型/联系电话/问题描述），联系人姓名自动从登录用户 realName 填充，附件槽位删除。投诉类型通过消息气泡内嵌 4 个点击按钮选择。

### 菜单结构

```
💬 智能问答 (/)           — 所有用户
👤 坐席工作台 (/operator)  — ADMIN/OPERATOR
📋 投诉建议 (/tickets)     — 所有用户
⚙ 系统管理
  ├─ 📚 知识库 (/knowledge)
  ├─ 🛡 敏感词管理 (/admin/sensitive)
  ├─ 👥 用户权限 (/admin)
  └─ 📝 审计日志 (/admin/audit)
```

侧边栏手风琴展开子菜单，Admin 页面只保留用户管理+角色管理两张卡片。

### 注册功能

登录页底部"申请注册" → 弹窗表单 → `POST /api/v1/auth/register`（白名单端点）。默认分配 USER 角色，需要验证码校验。
...
| GET | `/chat/sessions/{id}` | JWT | 会话历史详情（含坐席消息） |
| DELETE | `/chat/sessions/{id}` | JWT | 删除会话（软删除） |
| POST | `/knowledge/documents` | JWT | 上传知识文档 |
| GET | `/knowledge/documents` | JWT | 文档列表 |
| GET | `/knowledge/documents/{id}/chunks` | JWT | 文档分块列表 |
| POST | `/tickets` | JWT | 创建工单 |
| GET | `/tickets/mine` | JWT | 我的工单（ADMIN/OPERATOR 看全量） |
| GET | `/tickets/{ticketNo}` | JWT | 工单详情 |
| POST | `/auth/register` | 无 | 用户注册 |
| **坐席接口** | | | |
| GET | `/operator/tickets/pending` | JWT | 待处理工单池 |
| GET | `/operator/tickets/processing` | JWT | 我的处理中工单 |
| GET | `/operator/tickets/all` | JWT | 全量工单 |
| POST | `/operator/tickets/{no}/claim` | JWT | 认领工单 |
| PUT | `/operator/tickets/{no}` | JWT | 提交处理 |
| GET | `/operator/sessions/waiting` | JWT | 等待接入列表 |
| GET | `/operator/sessions/my` | JWT | 我的活跃会话 |
| POST | `/operator/sessions/{id}/takeover` | JWT | 接管会话 |
| POST | `/operator/sessions/{id}/message` | JWT | 坐席发消息 |
| POST | `/operator/sessions/{id}/close` | JWT | 关闭会话 |
| GET | `/admin/sensitive-words/list` | JWT+ADMIN | 敏感词列表 |
| POST | `/admin/sensitive-words/reload` | JWT+ADMIN | 敏感词热加载 |
| WS | `/ws/chat` | — | 人工对话 WebSocket |
...
| ↳ `operatorId` | 接管坐席ID | 1800s | 0 |
| ↳ `handoffSummary` | 转人工LLM摘要JSON | 1800s | 0 |
...
| `queue:operator_waiting` | 人工排队队列 | — | 0 |
| `operator:{id}:sessions` | 坐席活跃会话集合 | — | 0 |
...
详见项目根目录 [TODO.md](../TODO.md)，按 P0/P1/P2/P3 优先级排列。
